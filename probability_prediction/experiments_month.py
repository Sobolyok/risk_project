...
def print_scores(
    ...
    if threshold is None:
        ...
        for tmp_threshold in np.arange(0, 1, 0.001):
            f1 = f1_score((y_true > tmp_threshold), (y_predict > tmp_threshold), zero_division=1)
        ...
        f1 = f1_score((y_true > threshold), (y_predict > threshold), zero_division=1)
...
def main() -> NoReturn:
    ...
    x_train, x_test = x.loc[:"2019-10"], x.loc["2019-11"]
    ...
    plt.title(f"Probability that price will fall by {percent * 100}% relative to previous month average price")
    plt.show()
